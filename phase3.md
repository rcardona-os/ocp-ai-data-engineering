...
- 1.2. Select the **Workbenches** tab and click **Create workbench**.

   ![](media/wb0.png)

- 1.3. **Image Selection:** Select the *Standard Data Science* or *PyTorch* notebook image.

   ![](media/wb1.png)
   
   > **Note:** These images include pre-baked CLI tools like `boto3`, `oc`, and `pip`.

- 1.4. **Deployment Configuration:** * **Container Size:** Select a resource profile (e.g., *Medium: 1 CPU, 8Gi RAM*). **Persistent Storage:** Define a Persistent Volume Claim (PVC) size (minimum **20Gi** recommended for local caching).

   ![](media/wb2.png)

- 1.5. **Data Connection Association:** * Under the **Data connections** section, select **Use existing data connection**.
...
- 2.1. Click **Open** to launch the JupyterLab IDE.

   ![](media/wb6.png)
  
- 2.2. Open a **New Terminal** (`File` -> `New` -> `Terminal`).

   ![](media/wb7.png)

- 2.3. Execute the following command to check for the injected S3 metadata:
